...
full_name = "texture_CT+PET_concatenating"
feature_length = 16+16
...
def generate_x_y():
    ...
    for subject_id in range(len(subjectList)):
        ...
        data1 = loadmat("texture/{sub}_CT.mat".format(sub=subject))["data"].reshape(-1)
        data2 = loadmat("texture/{sub}_PET.mat".format(sub=subject))["data"].reshape(-1)
        x[subject_id] = np.hstack((data1, data2))
