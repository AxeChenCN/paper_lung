...
full_name = "intensity_CT+PET_wsvoting"
feature_length = 9
...
def generate_x_y():
    ...
    for subject_id in range(len(subjectList)):
        ...
        data1 = loadmat("{f}/{sub}_CT.mat".format(f="intensity", sub=subject))["data"].reshape(-1)
        data2 = loadmat("{f}/{sub}_PET.mat".format(f="intensity", sub=subject))["data"].reshape(-1)
...
for iterCount in range(10):
    ...
    for trainIndex, testIndex in trainAndTestIndex:         # 十折交叉验证
        ...
        for mol_id in range(len(weights)):
            y_score = y_score + y_scores[mol_id] * (weights[mol_id]/np.sum(weights))
        res = np.ones(y_score.shape)
        res[y_score < 0] = 0
        ACC, SEN, SPE = model_evaluate(test_label=y_test, res_label=res)
