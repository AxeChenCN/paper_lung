...
def generate_x_y():
    ...
    for subject_id in range(len(subjectList)):
        ...
        data = loadmat("{f}/{sub}_{m}.mat".format(f=feature_name, sub=subject, m=modality))["data"].reshape(-1)
...
for iterCount in range(10):
    ...
    for trainIndex, testIndex in trainAndTestIndex:         # 十折交叉验证
        ...
        rfe.fit(X_train_nol, y_train)
        selected = rfe.support_
        savemat("selected/selected_{f}_{m}_{c}_{fold}.mat".format(f=feature_name, m=modality, c=iterCount, fold=fold), {'data': selected})
...
if not os.path.exists("ROC"):
    os.mkdir("ROC")
savemat("ROC/{f}_{m}_fpr.mat".format(f=feature_name, m=modality), {'data': mean_fpr})
savemat("ROC/{f}_{m}_tpr.mat".format(f=feature_name, m=modality), {'data': mean_tpr})
