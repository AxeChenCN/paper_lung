...
full_name = "texture_CT+PET_wsvoting"
feature_length = 16
...
def generate_x_y():
    ...
    for subject_id in range(len(subjectList)):
        ...
        data1 = loadmat("{f}/{sub}_CT.mat".format(f="texture", sub=subject))["data"].reshape(-1)
        data2 = loadmat("{f}/{sub}_PET.mat".format(f="texture", sub=subject))["data"].reshape(-1)
